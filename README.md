...
* second, `match` the trace files' requests to our realblob dataset. So that each request will send a realblob or get a realblob to or from registry. In the end, `match` will generate `total_trace_registryname.json-realblob.json`, which is the trace file we are going to replay next and our code will automatically pickup this file.
