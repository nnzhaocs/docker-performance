...
python create_yaml_7node.py -r $1 -t $2 -m $3 -s $4 -a $5 -n $6 -c $7 -p $8
...
if [ $3 == "restore" ] && [ $9 == "preconstruct" ]; then
		...
		./run_sifttest.sh nnzhaocs/distribution:7nodepreconstruct  dedupregistries.txt $cachesize $repullthres
		...
		cachesize=$(echo "${sizearr[$2]}*1024*$cachesizeratio/$6+1"|bc)
                ...
                #imageno=$(echo "$6/7-1"|bc)
		#nocache
		./run_sifttest.sh nnzhaocs/distribution:7nocache  dedupregistries.txt $cachesize $repullthres
elif [ $3 == "restore" ] && [ $9 == "normallcache" ]; then
        cachesize=$(echo "${sizearr[$2]}*1024*$cachesizeratio/$6+1"|bc)
        echo "cachesize:"
        echo $cachesize
        #imageno=$(echo "$6/7-1"|bc)
        ./run_sifttest.sh nnzhaocs/distribution:7normallcache dedupregistries.txt $cachesize $repullthres
